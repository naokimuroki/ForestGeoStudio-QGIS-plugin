...
""" if opts.get("external_tile") else ""
        ...
        # ---- 空中写真 2時期比較変化解析（RGBタイル差分・近赤外なし）オプション ----
        # オプション1の保護JSとして読み込むが、WEB側UIはSentinel-2解析と同じく
        # 左コントロールのツールボタン + サブパネルで操作する。
        if opts.get("airphoto", False) and opt1_ok:
            _need_opt1("airphoto-change.js")
            airphoto_cfg = {
                "theme": {"main": theme["main"], "dark": theme["dark"]}
            }
            airphoto_init_js = (
                "\n  // 空中写真 2時期比較変化解析（オプション1）: airphoto-change.js があれば有効化\n"
                "  if (typeof AirphotoChangeExtension !== 'undefined') {\n"
                "    try { AirphotoChangeExtension.enable(map, %s); }\n"
                "    catch(e){ console.warn('[airphoto] init failed', e); }\n"
                "  }\n"
            ) % json.dumps(airphoto_cfg, ensure_ascii=False)
